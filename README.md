...
- **Offers to commit + push** the freshly-built MEX + shared-library artifacts back to `DYNAM-O/rust_bridge/`, tagged with platform name and dynamo_rs source SHA. Refuses to push to anything other than `rust-bridge` without explicit confirmation. See *Share pre-built binaries* below.
- **Offers to run `benchmark_runDYNAMO`** on the full night fixture (both backends, warmup + timed). Writes a per-run JSON under `DYNAM-O/rust_bridge/benchmarks/runs/` encoding hostname + os + arch + CPU + cores + RAM + MATLAB version + both repo SHAs + per-backend timings + peak counts, then commits and pushes just that one file. Takes ~3–6 min; skippable. See *Benchmarking* below.
...
git clone --recursive -b rust-bridge git@github.com:preraulab/DYNAM-O.git DYNAM-O
...
git clone --recursive -b rust-bridge https://github.com/preraulab/DYNAM-O.git DYNAM-O
...
# Existing checkouts from before 2026-04-24: DYNAM-O's submodule URLs
...
(cd DYNAM-O && git submodule sync && git submodule update --init --recursive)
...
When the bootstrap finishes building MEX, it diffs `DYNAM-O/rust_bridge/` against HEAD, shows you the new `.mex*` + shared-library files, and offers:
```
...
**Decline** if you're on a throwaway branch, lack push permission, or need to inspect the diff first. The commit is always made locally; the push is a second prompt. You can always push manually later with `cd DYNAM-O && git push origin rust-bridge`.

### Benchmarking

After the MEX step, the bootstrap offers to run `benchmark_runDYNAMO('night')` head-to-head on **both** backends — pure-MATLAB reference vs Rust MEX — and commits the resulting JSON under `DYNAM-O/rust_bridge/benchmarks/runs/`. The file captures:
...
See [`DYNAM-O/rust_bridge/benchmarks/README.md`](https://github.com/preraulab/DYNAM-O/blob/rust-bridge/rust_bridge/benchmarks/README.md) for the full JSON schema and contribution flow.
...
```bash
git clone --recursive -b rust-bridge https://github.com/preraulab/DYNAM-O.git
cd DYNAM-O
git submodule update --init --recursive
```
...
distribution notes:
[`DYNAM-O/rust_bridge/README.md`](https://github.com/preraulab/DYNAM-O/blob/rust-bridge/rust_bridge/README.md).
...
[`DYNAM-O/README.md`](https://github.com/preraulab/DYNAM-O/blob/rust-bridge/README.md).
...
MEX wrappers in [`DYNAM-O/rust_bridge/`](https://github.com/preraulab/DYNAM-O/tree/rust-bridge/rust_bridge) link against these artifacts at build time.
...
├── matlab/   → DYNAM-O  (MATLAB toolbox, GUI, File Manager)
